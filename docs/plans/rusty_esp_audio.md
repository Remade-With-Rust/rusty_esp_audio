...
| **A2** ◐ host half 2026-09-02 | ES8311 + ES7210 on Korvo-2 / S3-EYE; speaker out; loopback. **Done on the host:** `chip::{es8311, es7210}` — register maps, the clock-coefficient tables (75 and 25 rows) and every sequence the vendor drivers perform (init, sample-rate clocking, format and bit width, start, stop, volume, mic gain, mic select with TDM, mute) as data over `embedded-hal` 1.0 `I2c`, re-derived from Espressif's `esp-adf` `audio_hal` drivers (ESPRESSIF MIT License, attributed in each module) and reproduced register for register in tests against a fake bus; `CodecChip` is the trait a firmware programs against | mic → gain → speaker loopback with a measured latency; register tables re-derived and attributed — **the tables are; the loopback waits for a Korvo-2 or S3-EYE** |
