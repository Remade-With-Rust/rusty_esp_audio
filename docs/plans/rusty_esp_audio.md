...
|---|---|---|
| **A0** ✅ 2026-09-01 | elements, `RingBuffer`, `Pipeline`, `adpcm_ima`, `wav`, format conversions, host tests with synthetic PCM and, through `JANUS_SPEECH_WAV`, recorded speech (VAD within 3 points of ffmpeg `silencedetect`, AGC p90 at the target on three clips) | **passed:** biquads within 1 LSB of ffmpeg (f64 DF1) and scipy `lfilter`; IMA ADPCM **byte-identical to ffmpeg in both directions** (mono and stereo), PCM conversions byte-identical to swresample; AGC/VAD/DC-block/resampler property tests; riscv32 both rungs green; 45 + 3 + 6 tests. `docs/LEDGER.md` |
...
| **A2** ◐ host half 2026-09-02 | ES8311 + ES7210 on Korvo-2 / S3-EYE; speaker out; loopback. **Done on the host:** `chip::{es8311, es7210, es8388, es8156, es7243e}` (Korvo-2 / S3-BOX / S3-EYE, LyraT, S3-BOX-Lite) — register maps, the clock-coefficient tables (75 and 25 rows; the ES8388 takes MCLK/LRCK ratio codes instead) and every sequence the vendor drivers perform (init, sample-rate clocking, format and bit width, start, stop, volume, mic gain, mic select with TDM, mute) as data over `embedded-hal` 1.0 `I2c`, re-derived from Espressif's `esp-adf` `audio_hal` drivers (ESPRESSIF MIT License, attributed in each module) and reproduced register for register in tests against a fake bus; `CodecChip` is the trait a firmware programs against | mic → gain → speaker loopback with a measured latency; register tables re-derived and attributed — **the tables are; the loopback waits for a Korvo-2 or S3-EYE** |
